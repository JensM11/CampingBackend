...
namespace CampingAPI2.Data
{
    public class DataBase : IDataContext
    {
        ...
        {
            var campingsites = db.GetCollection<CampingSite>("CampingSites");
            return campingsites.Delete(name);
        }

        public async Task<IEnumerable<CampingSite>> GetCampingSitesByOwnerId(int ownerId)
        {
            var campingsites = db.GetCollection<CampingSite>("CampingSites");
            return campingsites.Find(c => c.OwnerId == ownerId);
        }
